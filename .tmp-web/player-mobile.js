(() => {
  window.JukuPlayerMobile = ({panel, video, state, showEpisodes, showPreferences}) => {
    ...
    const stage = panel.querySelector('.playback-stage');
    let scrubbing = false, qualitySignature = '', lastTitle = '', waiting = false, hideTimer = null, previousLandscape = false, tapStart = null;
    ...
    function reveal() {
      ...
      hideControls(false);
      if (video.paused || waiting || state().loading || scrubbing || panel.classList.contains('episodes-open') || panel.classList.contains('preferences-open') || node('playbackError').textContent) return;
      hideTimer = setTimeout(() => {if (panel.open && !video.paused && !scrubbing) hideControls(true);}, 3000);
    }
    ...
    stage.addEventListener('pointerup', event => {
      if (!tapStart || tapStart.id !== event.pointerId || event.target.closest('button,input,select,a,label')) {
        tapStart = null;
        return;
      }
      ...
      tapStart = null;
      if (distance > 12 || elapsed > 320 || event.defaultPrevented) return;
      event.preventDefault();
      if (panel.classList.contains('player-controls-hidden')) reveal();
      else if (video.paused) {reveal(); togglePlayback();}
      else {clearTimeout(hideTimer); hideControls(true);}
    });
    for (const type of ['pointercancel', 'lostpointercapture']) stage.addEventListener(type, () => {tapStart = null;});
